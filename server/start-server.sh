...
echo -e "${CYAN}╔════════════════════════════════════════════════════════╗${NC}"
echo -e "${CYAN}║${NC}        ${GREEN}个人博客系统 - 智能启动脚本 v2.5${NC}           ${CYAN}║${NC}"
echo -e "${CYAN}╚════════════════════════════════════════════════════════╝${NC}"
echo ""
...
check_and_install_dependencies() {
    ...
    if [ ! -f "package.json" ]; then
        echo -e "       ${RED}✗${NC} package.json 不存在，无法安装依赖"
        return 1
    fi
    
    if [ -d "node_modules" ] && [ "$force" != "force" ]; then
        local node_modules_size=$(du -sh node_modules 2>/dev/null | cut -f1)
        if [ -n "$node_modules_size" ] && [ "$node_modules_size" != "0K" ]; then
            echo -e "       ${GREEN}✓${NC} 依赖已安装 (${node_modules_size})"
            return 0
        fi
    fi
    
    if [ -d "node_modules" ]; then
        echo -e "       ${YELLOW}→${NC} 检测到损坏的依赖目录，重新安装..."
    else
        echo -e "       ${YELLOW}→${NC} 依赖未安装，正在安装..."
    fi
    
    npm install > "$LOG_DIR/npm-install-${name}.log" 2>&1
    
    if [ $? -eq 0 ]; then
        local size=$(du -sh node_modules 2>/dev/null | cut -f1)
        echo -e "       ${GREEN}✓${NC} 依赖安装成功 (${size})"
        return 0
    else
        echo -e "       ${RED}✗${NC} 依赖安装失败，查看日志: tail -f $LOG_DIR/npm-install-${name}.log"
        return 1
    fi
}
...
start_backend() {
    ...
    check_and_install_dependencies "$SCRIPT_DIR" "backend" || return 1
...
start_frontend() {
    ...
    check_and_install_dependencies "$PROJECT_DIR" "frontend" || return 1
